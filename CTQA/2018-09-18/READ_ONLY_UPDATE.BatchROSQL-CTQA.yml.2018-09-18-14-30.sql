/****** make backup table ********/
/* script creation time and backup table id: 18091818 */
create table CTQA_USER.SAKAI_REALM_RL_FN_18091818 as select * from CTQA_USER.SAKAI_REALM_RL_FN;
/****** initial count ******/

SELECT COUNT(*) FROM CTQA_USER.SAKAI_REALM_RL_FN;


CREATE TABLE CTQA_USER.SAKAI_REALM_RL_FN_T_18091818 AS
   SELECT * FROM (WITH
     excluded_site_id 
       AS (
 ...
 UNION  (SELECT site_id FROM   CTQA_USER.sakai_site  WHERE  site_id = 'fe189c68-2bff-4703-a56d-ce76dbdca12b')),
      candidate_site_id 
        AS (
            ...
            WHERE
                CTQA_USER.sakai_site.type IN ('specialized_projects','specialized_project','specialized_course','specialized_courses')),
     target_site_id
       AS (
...
DELETE FROM CTQA_USER.sakai_realm_rl_fn WHERE (role_key, realm_key, function, key) in (SELECT role_key, realm_key, function_key from CTQA_USER.SAKAI_REALM_RL_FN_T_18091818);
