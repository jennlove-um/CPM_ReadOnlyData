/****** make SAKAI_REALM_RL_FN backup table ********/
/* script creation time and backup table id: 18091917 */
create table CTQA_USER.SAKAI_REALM_RL_FN_18091917 as select * from CTQA_USER.SAKAI_REALM_RL_FN;
/****** make CPM_ACTION_LOG backup table ********/
/* script creation time and backup table id: 18091917 */
create table CTQA_USER.CPM_ACTION_LOG_18091917 as select * from CTQA_USER.SCPM_ACTION_LOG;
/****** initial count ******/

SELECT COUNT(*) FROM CTQA_USER.SAKAI_REALM_RL_FN;


CREATE TABLE CTQA_USER.SAKAI_REALM_RL_FN_T_18091917 AS
   SELECT * FROM (WITH
     excluded_site_id 
       AS (
               (SELECT site_id FROM   CTQA_USER.sakai_site  WHERE  site_id = '0143bc53-cd85-4785-bff8-ea91de2b51ee')
 ...
 UNION  (SELECT site_id FROM   CTQA_USER.sakai_site  WHERE  site_id = 'fe189c68-2bff-4703-a56d-ce76dbdca12b')),
      candidate_site_id 
        AS (
            ...
            WHERE
                CTQA_USER.sakai_site.type IN ('specialized_projects','specialized_project','specialized_course','specialized_courses','project',
'course')),
     target_site_id
       AS (
...
'disc.new.topic','disc.revise.any','disc.revise.own','lessonbuilder.upd')),
   ...
   -- list grant rows to act on.
    SELECT SRRF_2.*
          FROM   CTQA_USER.sakai_realm_rl_fn SRRF_2,
...
INSERT INTO CTQA_USER.cpm_action_log(ACTION_TIME,SITE_ID,ACTION_TAKEN) 
WITH
     excluded_site_id 
       AS (
               (SELECT site_id FROM   CTQA_USER.sakai_site  WHERE  site_id = '0143bc53-cd85-4785-bff8-ea91de2b51ee')
...
'course')),
     target_site_id
       AS (
          ...
          LEFT OUTER JOIN excluded_site_id
          ON candidate_site_id.site_id = excluded_site_id.site_id
          WHERE excluded_site_id.site_id is null)           select CURRENT_TIMESTAMP AS ACTION_TIME, SITE_ID AS SITE_ID, 'READ_ONLY_UPDATE' AS ACTION_TAKEN from dual,target_site_id ;
