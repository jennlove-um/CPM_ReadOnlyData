/****** make backup table ********/
/* script creation time and backup table id: 20180727 */
create table CTQA_USER.SAKAI_REALM_RL_FN_20180727 as select * from CTQA_USER.SAKAI_REALM_RL_FN;
/****** initial count ******/
...
SELECT COUNT(*) FROM CTQA_USER.SAKAI_REALM_RL_FN;
   ...
   DELETE 
   FROM   CTQA_USER.sakai_realm_rl_fn SRRF
   WHERE  EXISTS 
       ( SELECT 1
         FROM
           ( WITH 
     ...
     excluded_site_id 
       AS (
               (SELECT site_id FROM   CTQA_USER.sakai_site  WHERE  site_id = 'f2867e81-4954-4c27-ab55-fc9dd545bcf8')
